...
class BacktestService:
    ...
    async def create_backtest(
        ...
        try:
            backtest = await self.prisma.backtestrun.create(data={
                "userId": user_id,
    ...
    async def get_backtest_results(self, backtest_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": backtest.status,
                "created_at": backtest.createdAt.isoformat(),
                "completed_at": backtest.completedAt.isoformat() if backtest.completedAt else None,
                "results": results
            }
        finally:
            await self.prisma.disconnect()
    ...
    async def delete_backtest(self, backtest_id: int, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.prisma.backtestrun.delete(
                where={"id": backtest_id}
            )
    ...
    async def start_paper_trading(
        ...
        try:
            session = await self.prisma.papertradingsession.create(data={
                "userId": user_id,
                "strategyName": strategy_name,
                "symbol": symbol,
                "timeframe": timeframe,
    ...
    async def stop_paper_trading(
        ...
        try:
            ...
            if not session:
                raise ValueError("Paper trading session not found or does not belong to user")
            ...
            pnl = (updated_session.currentBalance or updated_session.virtualBalance) - updated_session.virtualBalance
            ...
            return {
                ...
                "final_balance": updated_session.currentBalance or updated_session.virtualBalance,
                "pnl": round(pnl, 2),
                "return_pct": round(return_pct, 2),
                "stopped_at": updated_session.stoppedAt.isoformat() if updated_session.stoppedAt else None
            }
        finally:
            await self.prisma.disconnect()
    ...
    async def get_paper_trading_status(
        ...
        try:
            ...
            pnl = (session.currentBalance or session.virtualBalance) - session.virtualBalance
            ...
            return {
                ...
                "return_pct": round(return_pct, 2),
                "started_at": session.startedAt.isoformat(),
                "stopped_at": session.stoppedAt.isoformat() if session.stoppedAt else None
            }
        finally:
            await self.prisma.disconnect()
