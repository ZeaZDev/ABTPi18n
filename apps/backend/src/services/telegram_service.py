...
class TelegramService:
    ...
    async def link_telegram_account(self, user_id: int, chat_id: str, username: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_link and existing_link.userId != user_id:
                ...
                return {"success": False, "error": "Chat ID already linked to another user"}
    ...
    async def unlink_telegram_account(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            link = await self.prisma.telegramlink.find_first(
                where={"userId": user_id}
            )
            ...
            await self.send_message(
                link.chatId,
                "👋 Your Telegram account has been unlinked from ABTPro."
            )
            
            # Delete link
            await self.prisma.telegramlink.delete(
                where={"userId": user_id}
            )
