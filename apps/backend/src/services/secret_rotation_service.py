...
class SecretRotationService:
    ...
    def __init__(self):
        ...
        self.grace_period_days = int(os.getenv("SECRET_ROTATION_GRACE_PERIOD_DAYS", "7"))
    ...
    async def create_rotation_record(
        ...
        # Hash the new secret value
        secret_hash = self.hash_secret(new_secret_value)
    ...
    async def update_rotation_policy(
        ...
        if not rotation:
            raise ValueError(f"No active rotation found for {secret_type}:{secret_name}")
        ...
        updated = await self.prisma.secretrotation.update(
            ...
            data={"nextRotation": new_next_rotation}
        )
