...
class PluginLoader:
    ...
    def __init__(self):
        ...
        self.verify_signatures = os.getenv("PLUGIN_VERIFY_SIGNATURES", "false").lower() == "true"
            ...
            try:
                ...
                for ep in eps:
                    try:
                        ...
                        logger.info(f"Discovered plugin: {plugin_class.name} v{plugin_class.version}")
    ...
    async def register_plugin(
        ...
        try:
            ...
            existing = await self.prisma.plugin.find_first(
                where={"name": name}
            )
            ...
            if existing:
                ...
                # Create new plugin
                plugin = await self.prisma.plugin.create(data={
                    "name": name,
                    "version": version,
                    "type": plugin_type,
                    "entryPoint": entry_point,
    ...
    async def install_plugin_for_user(
        ...
        try:
            # Find plugin
            plugin = await self.prisma.plugin.find_first(
            ...
            if existing:
                ...
                user_plugin = await self.prisma.userplugin.create(data={
                    "userId": user_id,
                    "pluginId": plugin.id,
                    "enabled": True,
                    "config": json.dumps(config) if config else None
