...
class BreakoutStrategy(Strategy):
    ...
    def execute(self, ticker_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        highs = ticker_data.get("highs", closes)  # Fallback to closes if highs not available
        lows = ticker_data.get("lows", closes)    # Fallback to closes if lows not available
        ...
        recent_high = max(highs[-self.lookback-1:-1]) if len(highs) > self.lookback else max(highs[:-1])
        recent_low = min(lows[-self.lookback-1:-1]) if len(lows) > self.lookback else min(lows[:-1])
        ...
        signal = "HOLD"
        # Upward breakout
        if current_price > recent_high and volume_confirmed and self.last_signal != "BUY":
            signal = "BUY"
        # Downward breakout
        elif current_price < recent_low and volume_confirmed and self.last_signal != "SELL":
            signal = "SELL"
        ...
        if signal == "BUY":
            breakout_strength = (current_price - recent_high) / price_range if price_range > 0 else 0
        elif signal == "SELL":
            breakout_strength = (recent_low - current_price) / price_range if price_range > 0 else 0
        else:
            breakout_strength = 0
